...
public class LinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    //该类中定义了两个ReentrantLock锁：putLock和takeLock，分别用于put端和take端。
    ...
    private void enqueue(Node<E> node) {
        ...
        last = last.next = node;	//入队是为last再找个下家
    }
    ...
    private E dequeue() {
        ...
        Node<E> h = head;
        Node<E> first = h.next;	//出队是把head.next取出来，然后将head向后移一位
        h.next = h; // help GC
        head = first;
        E x = first.item;
        first.item = null;
