...
/*
 	...
 	concurrentHashmap主要是为并发设计，与Collections的包装不同，他不是采用全同步的方式，而是采用非锁get方式，
 通过数据的弱一致性带来性能上的大幅提升，同时采用分段锁的策略，提高并发能力
 */
/**
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    /*
 		...
 		HashEntry的key，hash采用final，可以避免并发修改问题，HashEntry链的尾部是不能修改的，而next和value采用volatile，
 	可以避免使用同步造成的并发性能灾难，新版（jdk1.7）的concurrentHashmap大量使用java Unsafe类提供的原子操作，
 	直接调用底层操作系统，提高性能（这块我也不是特别清楚）
     */
    /**
    ...
    /*
     	...
     	那么必须保证segments的大小是2的整数次幂（power-of -two size）,concurrencyLevel默认是16，那么sshift默认就是4即左移的次数，
     	ssize为16。再啰嗦一点，默认有16个线程同时修改，那么我就默认的将segments大小设置为16，最理想情况下每个线程对应一个segment这样就不冲突了。
     	再啰嗦就是用户如果指定concurrencyLevel为13,14，ssize都会是16，就是一定要保证大小是不小于concurrencyLevel的最小的2的整数次幂。
    ...
    public ConcurrentHashMap(int initialCapacity,
        ...
        /*
		         这里给举例解释一下（都是在默认情况下，concurrencyLevel=16,ssize=16,sshift=4）那么   
	 	this.segmentShift = 32 - sshift=28 而this.segmentMask = ssize - 1=15二进制就是1111;
        ...
        /*
         	initialCapacity是总容量，ssize知道了是segments数组的大小，那么c显然就是每个segment的大小了（这不严谨），
     	实际上cap才是每个segment的容量大小，每个容量都是2的整数次幂。最少也是2
         */
        ...
        /*
         	一旦指定了concurrencyLevel（segments数组大小）便不能改变，这样，一旦threshold超标，
         	rehash真不会影响segments数组，这样，在大并发的情况下，只会影响某一个segment的rehash而其他segment不会受到影响
		（put方法都要上锁）
         */
	...
	跟同事沟通过，无论是1.6还是1.7的实现，实际上都是一种乐观的方式，而乐观的方式带来的是性能上的提升，但同时也带来数据的弱一致性，
	如果你的业务是强一致性的业务，可能就要考虑另外的解决办法（用Collections包装或者像jdk6中一样二次加锁获取）
	http://ifeve.com/concurrenthashmap-weakly-consistent/
	这篇文章可以很好地解释弱一致性问题
     */
    /*
     get方法并没有调用锁的痕迹，那么是如何实现线程安全的呢？这就得益于volatile关键字，保证了共享的东西的可见性 
     */
    /**
	...
	注：个人理解，这里采用自旋锁可能作者是觉得在分段锁的状态下，并发的可能本来就比较小，
	并且锁占用时间又并不是特别长，因此自旋锁可以减小线程唤醒和切换的开销
     */
    /**
